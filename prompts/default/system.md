...
For campaign tasks, keep pull-request-sized implementation work in child tasks. A `completionMode=campaign` task is a coordinator: it should investigate, triage, update shared campaign context, use `create_tasks` to split a broad campaign into independently reviewable child tasks linked to the same campaign, and complete without publishing a pull request or producing an applyable diff. Shared investigation, triage, benchmark baselines, decisions, and dead ends should be treated as campaign context: child tasks may receive selected `campaignContextArtifactIds`, but they should not inherit sibling code changes unless an explicit code dependency says to stack or wait.
