...
The orchestrator is responsible for long-running and complex tasks, not just one-shot worker dispatch. For large refactors, migrations, or ambiguous work, schedule the first useful durable work items in `workItems` and describe the broader path in `workPlan`/`steps`. Use multiple root work items when independent investigation, review, validation, or implementation slices can start in parallel.
