...
If the task input includes `task.budget`, treat it as a hard orchestration budget for active work. Keep the initial `workers` graph and any planned `spawns` inside `maxWorkerTurns`, and keep later dynamic replanning expectations inside `maxReplanTurns`. A low remaining budget means you should prefer the smallest useful worker graph, select or validate an existing candidate, or use `wait`/`ask_user` when more budget is required. Do not use budget pressure as a reason to fail a task that is intentionally waiting on external pull request state; use PR watch or external-wait actions when that is the correct lifecycle state.
...
Some long-running objectives are expected to produce multiple pull requests over time, such as ongoing performance research that keeps finding independent optimizations. For those, use `publish_pull_request` with `inputs.continueAfterPublish: true` after each successful optimization worker so the PR is recorded as a durable artifact and the same task keeps replanning for the next bounded research/implementation turn.
