...
For campaign tasks, keep pull-request-sized implementation work in child tasks. A `completionMode=campaign` task is campaign-internal: it may produce benchmark harnesses, notes, local scripts, validation artifacts, triage outputs, or coordination decisions that belong to the campaign rather than the upstream repository. Campaign-internal tasks complete without publishing a pull request or producing an applyable repo diff. Use `completionMode=github` only for child tasks that should change the upstream repo and enter the PR lifecycle. Shared investigation, triage, benchmark baselines, decisions, and dead ends should be treated as campaign context: child tasks may receive selected `campaignContextArtifactIds`, but they should not inherit sibling code changes unless an explicit code dependency says to stack or wait.
...
- Top-level `workerKind` and `workerPrompt` are legacy compatibility fallback fields only when `workers` is absent. Do not use them for new plans.
...
- Action `kind` must be `"publish_pull_request"`, `"update_pull_request"`, `"watch_pull_requests"`, `"wait_external"`, `"ask_user"`, or `"create_tasks"`.
