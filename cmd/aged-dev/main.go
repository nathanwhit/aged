...
func main() {
	var (
		...
		artifactMinAge    = flag.Duration("workspace-artifact-cleanup-min-age", envutil.Duration("AGED_WORKSPACE_ARTIFACT_CLEANUP_MIN_AGE", time.Hour), "aged daemon retained workspace artifact cleanup minimum age")
		artifactInterval  = flag.Duration("workspace-artifact-cleanup-interval", envutil.Duration("AGED_WORKSPACE_ARTIFACT_CLEANUP_INTERVAL", 10*time.Minute), "aged daemon retained workspace artifact cleanup scan interval")
