...
func main() {
	var (
		...
		artifactMinAge    = flag.Duration("workspace-artifact-cleanup-min-age", envutil.Duration("AGED_WORKSPACE_ARTIFACT_CLEANUP_MIN_AGE", time.Hour), "minimum terminal worker age before retained workspace artifact cleanup")
		artifactInterval  = flag.Duration("workspace-artifact-cleanup-interval", envutil.Duration("AGED_WORKSPACE_ARTIFACT_CLEANUP_INTERVAL", 10*time.Minute), "interval between retained worker artifact cleanup scans")
