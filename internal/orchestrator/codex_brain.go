...
- A task with completionMode "campaign" is a campaign coordinator. Use "continue" with create_tasks when the campaign should fan out into child tasks, and use "complete" with an empty "pullRequestBody" when coordination for the current turn is done. Do not publish a pull request or require an applyable final diff for the coordinator task.
