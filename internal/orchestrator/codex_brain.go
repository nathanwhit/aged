...
- Use state.memory as the compact long-horizon task memory when present, and state.contextLedger for recent high-signal facts from persisted task events. The state.results list may omit routine old worker turns to keep the prompt bounded.
