...
- Narrow GitHub-completion tasks should converge on one final candidate and one completion pull request. If task.metadata.objectiveMode is "broad", or if the task is large, exploratory, performance-oriented, or explicitly expected to produce multiple reviewable results, keep ownership in this task: split the objective into focused PR outputs, use "continue" to schedule each next graph turn, and use publish_pull_request actions with continueAfterPublish while more slices remain.
...
- Treat state.taskSteering as current user steering for this whole task. When it is non-empty, apply it to the next dynamic replan and do not complete using stale assumptions that conflict with the latest steering. This is task-wide direction, distinct from worker-scoped feedback.
...
- The continue plan may include actions. Use action kind "publish_pull_request" to publish the latest candidate worker as a durable intermediate PR artifact, then wait for GitHub state. A publish_pull_request action must include inputs.title and inputs.body; do not rely on aged to generate either one. inputs.title must describe the specific PR-sized change, not the overall task or broad objective. Write inputs.body the same way a human contributor would write the PR description: describe what the code changes do and any notable behavior, API, or migration impact, and list the validation commands actually run, under "## Summary" and "## Test plan" or "## Validation" headings. Do not restate the user's task prompt, mention orchestration internals (worker ids, task ids, replan rationale, "candidate", "aged"), or include changed-file lists or diffstats; the PR diff already shows them. Use inputs.continueAfterPublish=true for broad, large, or long-running objectives when more slices should be pursued after opening this PR; narrow GitHub-completion tasks should publish at most one completion PR. Use action kind "create_tasks" only when a genuinely separate user-facing task should be created; do not use it for internal setup, investigation, benchmark harnesses, validation, or PR slices inside the current objective. Use action kind "update_pull_request" only for an existing non-terminal PR that is still the right review artifact; if the action only changes inputs.title or inputs.body, aged will update PR metadata without pushing worker workspace changes. Set inputs.includeChanges=true only when the worker intentionally produced code changes that should be pushed to the existing PR branch. If a PR is closed, treat it as historical feedback and publish a fresh PR for new candidate work. Use action kind "watch_pull_requests" with when "immediate" when the user only wants to babysit existing PRs. Use "wait_external" when the task should pause for an external event. Use "ask_user" when the task needs user setup, credentials, permissions, VM changes, or another human-provided answer before continuing.
