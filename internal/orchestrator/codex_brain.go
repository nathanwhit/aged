...
- Normal GitHub-completion tasks should converge on one final candidate and one completion pull request. If worker results reveal several independent reviewable changes, choose the best coherent slice for this task and put the rest in "Recommended Next Turns" or schedule follow-up tasks; do not complete or continue a normal GitHub-mode task by planning multiple pull requests.
- A task with completionMode "campaign" is campaign-internal. It may be a coordinator, benchmark harness, local script, validation artifact, triage output, or another campaign-only work product outside the upstream repo PR lifecycle. Use "continue" with create_tasks when it should fan out into child tasks, and use "complete" with an empty "pullRequestBody" when the campaign-internal work is done. Do not publish a pull request or require an applyable final repo diff for the task.
...
- The continue plan may include actions. Use action kind "publish_pull_request" to publish the latest candidate worker as a durable intermediate PR artifact, then wait for GitHub state. A publish_pull_request action must include inputs.body with the PR description to publish; do not rely on aged to generate one. Write inputs.body the same way a human contributor would write the PR description: describe what the code changes do and any notable behavior, API, or migration impact, and list the validation commands actually run, under "## Summary" and "## Test plan" or "## Validation" headings. Do not restate the user's task prompt, mention orchestration internals (worker ids, task ids, replan rationale, "candidate", "aged"), or include changed-file lists or diffstats; the PR diff already shows them. Use inputs.continueAfterPublish=true only for explicit long-running/campaign-style objectives that should keep producing later pull requests; ordinary GitHub-completion tasks should publish at most one completion PR and use follow-up tasks for independent later slices. Use action kind "create_tasks" when a campaign task should split work into independently reviewable child tasks; inputs.tasks must be an array of child task objects with title, prompt, optional workstreamId, optional completionMode ("campaign", "github", or "local"; omitted means "campaign"), optional dependsOn, and optional campaignContextArtifactIds. Use action kind "update_pull_request" when the latest candidate worker should update an existing PR branch or PR metadata before returning to monitoring. Use action kind "watch_pull_requests" with when "immediate" when the user only wants to babysit existing PRs. Use "wait_external" when the task should pause for an external event. Use "ask_user" when the task needs user setup, credentials, permissions, VM changes, or another human-provided answer before continuing.
