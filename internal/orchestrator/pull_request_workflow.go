...
Repeatedly inspect CI status, review comments, and mergeability. If checks fail or review comments request changes, diagnose the issue, make the required code changes in the repo, and report what changed. Leave code changes in the existing PR checkout so aged can update this PR; do not use aged-publish-pr for existing PR follow-up work. When the PR update action will push code changes, include a commitMessage that is a short subject describing the diff or addressed feedback, not local progress/status narration. Do not post PR status comments about local preparation, local validation, mergeability, or pending branch updates yourself; when an update should include a public follow-up comment, provide the exact comment text through the PR update action so aged posts it only after the branch or metadata update succeeds. If reviewer feedback is purely a question and no code change is needed, report the suggested concise reply instead of posting it directly. If the PR is green and no action is needed, report that it is ready. Do not merge unless the user explicitly asks for merge.
...
Inspect the current PR state, CI failures, review comments, and mergeability. Schedule the next bounded worker turn needed to fix the PR or report that it is ready. The worker should leave code changes in the existing PR checkout so aged can update this PR; do not use aged-publish-pr for existing PR follow-up work. If reviewer feedback asks to improve or update the PR title, description, or body and no code change is required, the worker should use aged-update-pr instead of gh pr edit so aged records a metadata-only update. When an update_pull_request action will push code changes, it should include inputs.commitMessage as a short subject describing the diff or addressed feedback, not local progress/status narration. The worker must not post PR status comments about local preparation, local validation, mergeability, or pending branch updates itself; when an update should include a public follow-up comment, the worker should provide the exact comment text through aged-update-pr --comment or the update_pull_request action comment field so aged posts it only after the branch or metadata update succeeds. If reviewer feedback is purely a question and no code change is needed, it should report the suggested concise reply instead of posting it directly. Keep this as the same long-running task objective; do not start a separate babysitter task.
