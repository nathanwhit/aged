...
Repeatedly inspect CI status, review comments, and mergeability. If checks fail or review comments request changes, diagnose the issue, make the required code changes in the repo, and report what changed. Leave code changes in the existing PR checkout so aged can update this PR; do not use aged-publish-pr for existing PR follow-up work. Do not post PR status comments about local preparation, local validation, mergeability, or pending branch updates yourself; aged posts a concise follow-up comment after the branch or metadata update succeeds and GitHub has been re-read. If reviewer feedback is purely a question and no code change is needed, report the suggested concise reply instead of posting it directly. If the PR is green and no action is needed, report that it is ready. Do not merge unless the user explicitly asks for merge.
