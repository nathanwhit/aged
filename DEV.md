...
- Scheduler plans now support `actions` so the orchestrator brain has finer-grained control over workflow shape. `publish_pull_request` opens a durable intermediate PR artifact after a successful worker turn and leaves the task waiting on external GitHub state; `wait_external` pauses an objective for non-GitHub external conditions.
...
- Pull request tests verify PR projection, status refresh, intermediate plan-action PR publication from worker workspaces without local apply, and legacy babysitter task scheduling.
