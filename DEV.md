...
- Pull request refresh updates the task's PR artifact and objective phase. Merged PRs record a `pr_merged` milestone and satisfy the task; closed unmerged PRs abandon/cancel the task. When a project's pull request policy has both `allowMerge` and `autoMerge`, the monitor squash-merges tracked PRs once they are open, non-draft, passing, review-clean, and mergeable.
