...
- Durable loop tasks periodically record a compact `task.memory_updated` snapshot with repeated iteration themes, important decisions, blockers, and PR/artifact state so late-loop workers receive long-horizon context in addition to the fixed-size event ledger.
