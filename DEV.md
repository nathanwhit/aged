...
- Runtime project creation is available through `POST /api/projects` and the dashboard Projects panel. Project registration validates `localPath` and discovers VCS kind / GitHub repo metadata from the checkout when possible.
...
- Project creation is DB-backed through the API/UI and now validates local paths plus basic checkout metadata. Editing, deletion, health checks, and default-branch discovery are still pending.
