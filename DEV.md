...
- Projects persist a `pullRequestPolicy` used by PR publication. The policy currently covers branch prefix, draft-by-default behavior, whether aged is allowed to merge, whether auto-merge is permitted, and the auto-merge method; repo/base/fork selection remains in the project repo/upstream/head-owner/push-remote/default-base fields.
...
- Built-in GitHub driver support is available with bare `-github-driver` / `AGED_GITHUB_DRIVER=true`, or with `-github-driver=path-or-json` / `AGED_GITHUB_DRIVER=path-or-json` for daemon-wide settings, and can be read or hot-swapped at runtime with `GET` / `PUT /api/drivers/github`. Projects can opt into issue polling with `githubIssues` and mention polling with `githubMentions`; the driver polls those project repos through `gh`, creates idempotent `github-issue` tasks routed back to the owning project, creates local-completion `github-mention` tasks for mention/review-request notifications, auto-publishes PRs for tasks that opt into GitHub completion mode, refreshes known PR status, and steers the original waiting task when checks fail, reviews request changes, or mergeability is blocked/dirty.
- Built-in Discord driver support is available with `-discord-driver` / `AGED_DISCORD_DRIVER` and can be read or hot-swapped at runtime with `GET` / `PUT /api/drivers/discord`; state responses redact the bot token. It polls configured channels with a bot token, ignores startup history by default, answers messages through the configured assistant, supports structured `answer` / `list_projects` / `create_project` / `propose_task` / `create_task` decisions, carries `projectId` / `title` / `prompt` into created tasks, can add persisted projects through chat, runs chat turns in the selected project checkout for read-only code inspection, recovers pending proposals from persisted assistant events, and still supports `task: <prompt>` plus `do it` shortcuts.
...
- Pull request refresh updates the task's PR artifact and objective phase. Merged PRs record a `pr_merged` milestone and satisfy the task; closed unmerged PRs abandon/cancel the task. When a project's pull request policy has both `allowMerge` and `autoMerge`, the monitor merges tracked PRs with the configured `mergeMethod` once they are open, non-draft, passing, review-clean, and not explicitly blocked or conflicting.
...
- Execution targets can also be added, edited, and deleted dynamically through `/api/targets` and the dashboard Targets pane. Dynamic targets are persisted in SQLite and registered with the live scheduler without restarting the daemon. The dashboard target form edits SSH `checkoutRoot`; the project form edits per-target `remoteCheckouts` as target id/path rows.
...
- Fresh SSH workers prepare a project-scoped checkout before launch: project `remoteCheckouts[targetId]` overrides the target path, otherwise aged derives `checkoutRoot/<project-id>`. Existing target `workDir` values are accepted as a compatibility alias for `checkoutRoot`. Missing checkouts are cloned from the configured project repo; existing Git checkouts fetch and detach at `origin/<defaultBase>`. Retries and dependency follow-ups reuse the previous remote workdir.
- SSH target health is probed in the background. Probes report reachability, tmux availability, checkout root preparation, disk, load/CPU, and memory; snapshots/UI show the current health/resource state.
...
- `benchmark_compare` provides a reusable primitive for explicit numeric before/after benchmark comparison. It supports same-command and same-target checks, repeated baseline/candidate samples, minimum sample counts, threshold checks, and median-based verdicts while retaining the simple single-number path.
...
- `publish_pull_request` actions wait by default for the opened PR's external lifecycle. Long-running research tasks can set `inputs.continueAfterPublish: true` to record an optimization PR and keep the same task active for subsequent PR-producing turns.
- Dynamic continuation plans can set `metadata.baseWorkerID` to `"source"` to start a fresh worker from the project source checkout instead of inheriting the latest changed candidate, which supports independent PR streams from one long-running task.
...
- Dynamic replanning is available for brains that implement `Replan`: after follow-up workers, the brain can return `continue`, `complete`, `wait`, or `fail`; `continue` schedules another worker turn and records `task.replanned`. There is no fixed replan turn cap; long-running objectives continue until they explicitly wait, complete, fail, or are canceled.
...
- Multi-turn orchestration executes initial and dynamically replanned `spawns` as dependency graphs. Dynamic replanning is intentionally unbounded for objectives that are expected to keep running indefinitely, with cancellation and wait states acting as the control points.
